...
function parsePropertyDecorators(node) {
    if (TypescriptGuards_1.isPropertyDeclaration(node)) {
        let decoratorNames = [];
        if (node.decorators && node.decorators.length > 0) {
            for (const decorator of node.decorators) {
                ...
                if (expression.arguments && expression.arguments.length > 0) {
                    decoratorString += '(' + expression.arguments.map((a) => a.text).join(',') + ')';
                }
    ...
    if (node.members) {
        node.members.forEach((o) => {
            if (TypescriptGuards_1.isPropertyDeclaration(o)) {
                ...
                if (o.modifiers) {
                    classDeclaration.properties.push(new PropertyDeclaration_1.PropertyDeclaration(o.name.text, parse_utilities_1.getNodeVisibility(o), parse_utilities_1.getNodeType(o.type), !!o.questionToken, parse_utilities_1.containsModifier(o, typescript_1.SyntaxKind.StaticKeyword), o.getStart(), o.getEnd(), decorators));
                }
